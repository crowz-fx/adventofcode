...
class Utils:
  ...
  def run_solution(self, 
    ...
    self.out("Executing solution...")
    
    if run_tests:
      self.out("Running tests...")
      for i in range(1, test_count + 1):
        self.out(f"Running test [{i}]...")
        test_result_actual = solution(self.file_contents(root_dir, f"test_{i}.txt"))
        test_result_expected = int(str(self.file_contents(root_dir, f"test_{i}_result.txt")[0]) \
            .replace("\n", "").strip())
        ...
        if test_result_actual == test_result_expected:
          ...
          self.err(f"Test [{i}] failed, expected [{test_result_expected}] but got [{test_result_actual}]!")
          self.error_out()
    ...
    if run_input:
      ...
      self.dun(f"Ouput from solution [{solution(self.file_contents(root_dir, 'input.txt'))}]")
